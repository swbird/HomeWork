...
async function writeAbiAddr(artifacts, addr, name, network){
  ...
  // let targetpath = `../deployments/${network}/`

  //   if (!fs.existsSync(targetpath)) {
  //     console.log(targetpath," 不存在, 正在创建")
...
async function writeLog(deployments, name, network){
    console.log("in writeLog");
    let file = `../deployments/${network}/${name}.json`;
    

    
    const deploymentPath = path.resolve(__dirname, file);
